...
* @author QUEENNICKY <https://github.com/QUEENNICKY>
* @description A WhatsApp based 3ʳᵈ party application that provide many services with a real-time automated conversational experience
* @link <https://github.com/QUEENNICKY/Queen-nicky>
* @version 4.0.7
* @file  downloadYT.js - QUEENNICKY YouTube downloader

© 2022 QUEEN NICKY, ANTECH. All rights reserved.
Licensed under the  GPL-3.0 License;
you may not use this file except in compliance with the License.*/

const { AMDI, Language, youtubeDL } = require('QUEEN_NICKY_core/dist/scripts')
...
NICKY({ cmd: ["song", "yta", "mp3"], desc: Lang.songDesc, example: Lang.songExa, type: "download", react: "🎵" }, (async (NICKYWA) => {
    ...
    if (!execYT.isYT) {
        ...
        const ytDlTXT = `*🎶 QUEEN NICKY YT Downloader*\n\n📄 ${Lang.Title} ${title}`
        const buttons = [
...
NICKY({ cmd: ["video", "ytv", "mp4"], desc: Lang.videoDesc, example: Lang.videoExa, type: "download", react: "🎞️" }, (async (amdiWA) => {
    let { input, prefix, reply, sendButtonsMsg, sendListMsg, urlRegexYT } = amdiWA.msgLayout;
    ...
    if (!execYT.isYT) {
        ...
        const title = ytVidList[0] ? ytVidList[0].title : ''
        const ytDlTXT = `*🎞️ QUEEN NICKY YT Downloader*\n\n📄 ${Lang.Title} ${title}`
        const buttons = [
            { type: "url", displayText: "Watch on YouTube", url: input },
...
NICKY({ cmd: "ytinfo", desc: Lang.YTINFO, type: "primary", react: "ℹ️" }, (async (amdiWA) => {
...
NICKY({ cmd: "ytdownload", cmdHideInMenu: true, type: "download" }, (async (amdiWA) => {
